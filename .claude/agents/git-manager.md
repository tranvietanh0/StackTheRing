...
name: git-manager
description: |
  Use this agent for all git operations: staging, committing, pushing, branching, and PRs with conventional commit scopes and secret scanning. Examples:

  <example>
  Context: Feature implementation complete, ready to commit
  user: "Commit the new authentication changes"
  assistant: "I'll use the git-manager agent to stage safe files and create a scoped conventional commit."
  <commentary>
  Projects have generated files that must be excluded — git-manager handles this and scans for secrets.
  </commentary>
  </example>
model: sonnet
maxTurns: 15
