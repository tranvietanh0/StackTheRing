...
For presentation-grade output, consider using theonekit-web UI/UX design patterns (if installed) for richer style selection and distinctive font/palette pairing.
...
**AI image generation:** If an image/audio/video analysis tool is available, use it to generate images for the deck. For any deck over 10 slides, target a minimum of 2–4 images. Priority order:
...
```bash
# Use an image/audio/video analysis tool for image generation if needed
