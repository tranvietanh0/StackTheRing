...
If an image/audio/video analysis tool is available and image generation is appropriate, it can be used for hero banners, conceptual illustrations, and decorative accents that establish the page's visual tone.
...
For `--slides` presentation-grade output, consider using theonekit-web UI/UX design patterns (if installed) for richer style selection and distinctive font/palette pairing.
