...
### Hardcoded Mappings (CRITICAL for T1K hooks)
- Static maps (command→skill, command→role, command→agent) that duplicate data from JSON/MD files — MUST read from registry files at runtime
- Inline `.claude/` path construction (`path.join(cwd, '.claude')`) instead of using `resolveClaudeDir()` from shared module
- Hardcoded string literals for file/directory names that exist as constants in `T1K` from `telemetry-utils.cjs`

### Duplicated Logic (HIGH PRIORITY)
- Same pattern implemented in 2+ files — extract to a shared module IMMEDIATELY
- Utility functions that duplicate existing ones in `telemetry-utils.cjs` or `lib/`
- Copy-pasted blocks with slight variation — unify with a parameter or shared abstraction
- `null`/`undefined` not guarded at data boundaries (JSONL → Set → JSON → DB)

### Silent Fallbacks & Error Swallowing
- Empty `catch {}` blocks that hide failures — at minimum log the error or rethrow with context
- `catch { process.exit(0) }` without any error output — add `process.stderr.write()` before exit
- Fallback values that mask broken state (e.g., returning `[]` instead of throwing when data source is missing)
- `|| null` / `|| ''` chains that silently degrade instead of surfacing the error
- Exception: hooks MAY fail-open (`catch { process.exit(0) }`) only if they are non-critical AND the fail-open is explicitly documented in a comment

### SOLID Violations
- **Single Responsibility:** function/class doing more than one thing — split it
- **Open/Closed:** modifying existing code instead of extending via composition
- **Dependency Inversion:** depending on concrete implementations instead of abstractions/interfaces
