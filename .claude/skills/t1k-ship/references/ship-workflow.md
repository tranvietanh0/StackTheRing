...
1. Spawn `journal-writer` subagent via `Task` tool in background to record a session summary:
...
1. Invoke `/t1k:docs update` via `docs-manager` subagent in background:
