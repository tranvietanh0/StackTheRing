...
// Main
function main() {
  // Extended commands handled by worktree-ops.cjs
  const extendedCommands = ['session', 'sync', 'envsync', 'diff', 'status'];
  if (extendedCommands.includes(command)) {
    const opsScript = path.join(__dirname, 'worktree-ops.cjs');
    const passArgs = process.argv.slice(2).map(a => `"${a}"`).join(' ');
    try {
      execSync(`node "${opsScript}" ${passArgs}`, { stdio: 'inherit', cwd: process.cwd() });
    } catch (err) {
  ...
  switch (command) {
    ...
    default:
      outputError('UNKNOWN_COMMAND', `Unknown command: ${command || '(none)'}`, {
        suggestion: 'Available commands: create, remove, info, list, session, sync, envsync, diff, status'
      });
  }
}
